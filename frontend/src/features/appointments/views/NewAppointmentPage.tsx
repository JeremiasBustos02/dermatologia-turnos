...
export const NewAppointmentPage = () => {
    ...
    const handleFinalSubmit = () => {
        ...
        createMutation.mutate({
            ...
            notes: 'Turno generado desde el Wizard'
        }, {
            onSuccess: () => {
                navigate('/dashboard', {
                    state: {
                        showSuccessToast: true,
                        toastMessage: '¡Turno reservado con éxito! Se ha enviado el comprobante por correo electrónico al paciente.'
                    }
                });
            },
            onError: (error: any) => {
                const backendError = error.response?.data;
                const errorMessage = backendError?.message || 'Error desconocido de conexión';
                ...
                alert(`NestJS dice:\n\n${finalMessage}`);
                console.error('Error completo:', backendError);
            }
        });
    };
                    ...
                    {STEPS.map((stepName, index) => {
                        ...
                        return (
                            <div key={stepName} className="relative z-10 flex flex-col items-center gap-2 bg-white px-2">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm transition-colors ${isActive ? 'bg-blue-600 text-white ring-4 ring-blue-100' :
