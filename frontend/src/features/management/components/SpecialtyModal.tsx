...
export const SpecialtyModal = ({ isOpen, onClose, onSubmit }: SpecialtyModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      setError('El nombre de la nomenclatura médica es obligatorio.');
      return;
    }
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-150">
        ...
        <div className="flex justify-between items-center p-4 bg-slate-50 border-b border-slate-200">
          <h3 className="text-xs font-bold text-slate-800">Alta de Especialidad</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs font-medium text-slate-700">
          ...
          <div className="space-y-1">
            <label className="text-slate-500 font-semibold">Nombre de la Especialidad *</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="Ej: Dermatología Clínica, Tricología"
          ...
          <div className="space-y-1">
            <label className="text-slate-500 font-semibold">Descripción del Alcance (Opcional)</label>
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Descripción breve de las patologías o tratamientos incluidos..."
